...
void PIDcontroller::setLimit(double _low_limit, double _high_limit)	{
...
float PIDcontroller::getOutput(float processVariable) {
	...
	//dt = dt> 0.01f?dt:0.01f;
	float total = proportionalGain + derivativeGain + integralGain;
	total = (total > high_limit)? high_limit: total;
	total = (total < low_limit)? low_limit: total;
